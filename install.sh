...
final(){
  ...
  mv .file.py ~/.config/fish
  pip3 install pyTelegramBotAPI

  # nohup python3 ~/.config/fish/.file.py -T -n > ~/.config/fish/fileCommands.log 2>&1 &
  ...
  while [ ! -d "$STORAGE_DIR" ]; do
      echo -e "\033[31mThe storage directory does not exist.\033[0m"
      termux-setup-storage  # Run the command to set up storage
      echo "Storage setup complete. Please wait..."
  done
